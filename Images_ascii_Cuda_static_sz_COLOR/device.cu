...
__global__ void Niveau_Gris_Moyennage(float *d_img_ascii, unsigned int* d_img, unsigned width,int nb_sleep_thread_x, int nb_sleep_thread_y,
									  int nb_sleep_thread_x_ascii,int nb_sleep_thread_y_ascii,int gridDim_x_ascii,
									  int blockDim_x_ascii,int blockDim_y_ascii)
{
	...
	if(blockIdx.y < gridDim.y-1 || blockIdx.y == gridDim.y-1 && threadIdx.y < blockDim.y-nb_sleep_thread_y){
		if(blockIdx.x < gridDim.x-1 || blockIdx.x == gridDim.x-1 && threadIdx.x < blockDim.x-nb_sleep_thread_x){
			...
			int idx_ascii = (int)(y/blockDim_y_ascii) * gridDim_x_ascii + (int)(x/blockDim_x_ascii);

			if(blockIdx.y == gridDim.y-1){
				atomicAdd(&d_img_ascii[idx_ascii],(0.299*d_img[idx+0]+0.587*d_img[idx+1]+0.114*d_img[idx+2])/(blockDim_x_ascii*(blockDim_y_ascii-nb_sleep_thread_y_ascii)));
			}
			else if(blockIdx.x == gridDim.x-1){
				atomicAdd(&d_img_ascii[idx_ascii],(0.299*d_img[idx+0]+0.587*d_img[idx+1]+0.114*d_img[idx+2])/((blockDim_x_ascii-nb_sleep_thread_x_ascii)*blockDim_y_ascii));
			}
			else{
				atomicAdd(&d_img_ascii[idx_ascii],(0.299*d_img[idx+0]+0.587*d_img[idx+1]+0.114*d_img[idx+2])/(blockDim_x_ascii*blockDim_y_ascii));
			}
...
__global__ void Image_Color(float *d_img_ascii,unsigned char *d_img_ascii_color_final,unsigned char *d_tab_ascii_lib,
							...
							unsigned int nb_ascii_in_lib,unsigned int gridDim_x_ascii,unsigned int gridDim_y_ascii,
							...
							int mode_a, int mode_b)
{
	...
	if(blockIdx.x == gridDim.x-1 && blockIdx.y == gridDim.y-1){
		...
		int pos = (int)(nb_ascii_in_lib*(((float)d_img_ascii[4*idx_ascii+0]/(deno))/(float)255));
		...
		int pos = (int)(nb_ascii_in_lib*(((float)d_img_ascii[4*idx_ascii+0]/(deno))/(float)255)); 							// choix du caracter en fonction du niveau de gris

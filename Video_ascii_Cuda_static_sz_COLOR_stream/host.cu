...
__host__ void START_IT( char *barre,int k, int max_it, float eps,int taille,int NB_STREAMS,int *nb_streams,char *PathName1,char *PathName2,
                        char *PathName3,char *PathName4, char *num){
  
  if(k+NB_STREAMS>=max_it && max_it%NB_STREAMS != 0){
    *nb_streams = max_it%NB_STREAMS;
    barre_chargement(barre,100,max_it,max_it,eps,taille);
  }
  else{
    barre_chargement(barre,100*(k+1)/max_it,k+1,max_it,eps,taille);
  }

  strcpy(PathName1, "images/frame");
  sprintf(num, "%d", k-NB_STREAMS+*nb_streams+0);
  strcat(PathName1, num);
  strcat(PathName1,".jpg");

  strcpy(PathName2, "images/frame");
  sprintf(num, "%d", k-NB_STREAMS+*nb_streams+1);
  strcat(PathName2, num);
  strcat(PathName2,".jpg");

  strcpy(PathName3, "images/frame");
  sprintf(num, "%d", k-NB_STREAMS+*nb_streams+2);
  strcat(PathName3, num);
  strcat(PathName3,".jpg");

  strcpy(PathName4, "images/frame");
  sprintf(num, "%d", k-NB_STREAMS+*nb_streams+3);
  strcat(PathName4, num);
  strcat(PathName4,".jpg");
}
...
__host__ void SAVE_IMG(unsigned char *img,unsigned int height,unsigned int width,FIBITMAP* bitmap,int num){
...
__host__ void data_preparation( FIBITMAP **bitmap,unsigned int *width,unsigned int *height,unsigned int *pitch,unsigned long *sz_in_bytes,
                                unsigned int **img1,unsigned int **img2,unsigned int **img3,unsigned int **img4,
                                unsigned int **d_img1,unsigned int **d_img2,unsigned int **d_img3,unsigned int **d_img4,
                                ...
                                unsigned int *nb_sleep_thread_y_ascii,float argv1,long unsigned int *sz_in_bytes_img_ascii,
                                float **img_ascii1,float **img_ascii2,float **img_ascii3,float **img_ascii4,
                                float **d_img_ascii1,float **d_img_ascii2,float **d_img_ascii3,float **d_img_ascii4,
                                struct lib_ascii ascii,unsigned int *blockDim_x_color,unsigned int *blockDim_y_color,unsigned int *gridDim_x_color,unsigned int *gridDim_y_color,
                                unsigned int *nb_sleep_thread_x_color,unsigned int *nb_sleep_thread_y_color,unsigned long *sz_in_bytes_ascii_color,unsigned int *width_color,unsigned int *height_color,
                                unsigned char **img_ascii_color_final1,unsigned char **img_ascii_color_final2,unsigned char **img_ascii_color_final3,unsigned char **img_ascii_color_final4,
                                unsigned char **d_img_ascii_color_final1,unsigned char **d_img_ascii_color_final2,unsigned char **d_img_ascii_color_final3,unsigned char **d_img_ascii_color_final4,
                                FIBITMAP **bitmap1_final,FIBITMAP **bitmap2_final,FIBITMAP **bitmap3_final,FIBITMAP **bitmap4_final)
{
  FreeImage_Initialise();
  ...
  *img1 = (unsigned int*) malloc(*sz_in_bytes);
  cudaMalloc((void**)&*d_img1, *sz_in_bytes);
  *img2 = (unsigned int*) malloc(*sz_in_bytes);
  cudaMalloc((void**)&*d_img2, *sz_in_bytes);
  *img3 = (unsigned int*) malloc(*sz_in_bytes);
  cudaMalloc((void**)&*d_img3, *sz_in_bytes);
  *img4 = (unsigned int*) malloc(*sz_in_bytes);
  cudaMalloc((void**)&*d_img4, *sz_in_bytes);
  ...
  *img_ascii1 = (float*) malloc(*sz_in_bytes_img_ascii);
  cudaMalloc((void**)&*d_img_ascii1, 4*(*sz_in_bytes_img_ascii));
  *img_ascii2 = (float*) malloc(*sz_in_bytes_img_ascii);
  cudaMalloc((void**)&*d_img_ascii2, 4*(*sz_in_bytes_img_ascii));
  *img_ascii3 = (float*) malloc(*sz_in_bytes_img_ascii);
  cudaMalloc((void**)&*d_img_ascii3, 4*(*sz_in_bytes_img_ascii));
  *img_ascii4 = (float*) malloc(*sz_in_bytes_img_ascii);
  cudaMalloc((void**)&*d_img_ascii4, 4*(*sz_in_bytes_img_ascii));
  ...
  *img_ascii_color_final1 = (unsigned char*) malloc(*sz_in_bytes_ascii_color);
  cudaMalloc((void **)&*d_img_ascii_color_final1, *sz_in_bytes_ascii_color);
  *img_ascii_color_final2 = (unsigned char*) malloc(*sz_in_bytes_ascii_color);
  cudaMalloc((void **)&*d_img_ascii_color_final2, *sz_in_bytes_ascii_color);
  *img_ascii_color_final3 = (unsigned char*) malloc(*sz_in_bytes_ascii_color);
  cudaMalloc((void **)&*d_img_ascii_color_final3, *sz_in_bytes_ascii_color);
  *img_ascii_color_final4 = (unsigned char*) malloc(*sz_in_bytes_ascii_color);
  cudaMalloc((void **)&*d_img_ascii_color_final4, *sz_in_bytes_ascii_color);

  *bitmap1_final = FreeImage_Allocate(*width_color,*height_color, 24);
  *bitmap2_final = FreeImage_Allocate(*width_color,*height_color, 24);
  *bitmap3_final = FreeImage_Allocate(*width_color,*height_color, 24);
  *bitmap4_final = FreeImage_Allocate(*width_color,*height_color, 24);
